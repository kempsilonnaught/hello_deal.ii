...
template <int dim>
void Coefficient<dim>::value_list(const std::vector<Point<dim>> &points, std::vector<double> &values,
								  const unsigned int component) const{
	Assert(values.size() == points.size(), ExcDimensionMismatch(values.size(), points.size()));

	Assert(components == 0, ExcIndexRange(component, 0, 1));
...
void Step5<dim>::solve(){
	...
	std::cout << "	" << solver_control.last_step() << " CG iterations need to obtain convergence." << std::endl;
